...
def AllTrue(LH,TIME_YC,YC_START,YC_END,FLAG=1):
    conn = cx_Oracle.connect("YGSJPT/YGSJPT@10.4.133.222/YGSJPT")    #此处需修改
    cursor=conn.cursor()
    param={"LH":LH,"TIME_YC":TIME_YC,"FLAG":FLAG,"YC_START":YC_START,"YC_END":YC_END}
    cursor.execute("insert into TB_DSJ_SJ(LH,TIME_YC,FLAG,YC_START,YC_END) values(:LH,:TIME_YC,:FLAG,:YC_START,:YC_END)",param)
    conn.commit()
    cursor.close()
    conn.close()
...
#     conn = cx_Oracle.connect("YGSJPT/YGSJPT@10.4.133.222/YGSJPT")    #此处需修改
#     cursor=conn.cursor()
#     # param={"LH":'%s'%(LH),"TIME_YC":TIME_YC,"FLAG":FLAG,"YC_START":YC_START,"YC_END":YC_END}
...
def main():
    while True:
        ...
        try:
            ...
            # conn = cx_Oracle.connect("YGSJPT/YGSJPT@10.4.133.222/YGSJPT")  # 此处需修改
            # cursor = conn.cursor()
            # param={"YC_START":YC_START,"YC_END":YC_END}
            # cursor.execute("insert into TB_DSJ_SJ(YC_START,YC_END) values(:YC_START,:YC_END) where LH='%s'"%(LH),param)
            # conn.commit()
            # cursor.close()
            # conn.close()
