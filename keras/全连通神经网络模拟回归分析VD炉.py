...
def SelectOracle(sql):
    conn = cx_Oracle.connect("system/Oracle11w@localhost/ORCL")
    # sql = "select * from VD"
    cursor = conn.cursor()
    cursor.execute(sql)
    OracleData = cursor.fetchall()
    columns = ["炉号", "包况", "包龄", "精炼冶时", "总送电时间", "钢种", "进工位温度",
               "破空温度", "出工位温度", "氩气流量", "抽真空时间", "温差"]
...
########################################################################################################################
#     #  从Oracle中提取数据  #
# conn=cx_Oracle.connect("systm/Oracle11w@localhost/ORCL")
# sql="select * from VD"
# cursor=conn.cursor()
# cursor.execute(sql)
# OracleData=cursor.fetchall()
...
# early_stopping=EarlyStopping(monitor='val_loss',patience=50,verbose=2)
#
# history=model.fit(train_X_scaler_fit_transform,train_y_scaler_fit_transform,epochs=300,batch_size=10,
#                   validation_data=(test_X_scaler_transform,test_y_scaler_transform),verbose=2,
#                   shuffle=False,callbacks=[early_stopping])
#
# os.chdir("C:/Users/Rooobins/Desktop/")
...
#
# #预测
# predict_y=model.predict(test_X_scaler_transform)
#
# #预测y 逆标准化
# predict_y=train_y_scaler_fit.inverse_transform(predict_y)
#
#
# #原始y 逆标准化
# _y=train_y_scaler_fit.inverse_transform(test_y_scaler_transform)
#
# #计算RMSE
# rmse=sqrt(mean_squared_error(predict_y,_y))
# print("Test RMSE : %.3f"%rmse)
# plt.plot(_y,label="真实值")
# plt.plot(predict_y,label='预测值')
# plt.legend()
# plt.show()
#
# mark=np.linspace(1,len(test_X),len(test_X))
# plt.scatter(mark,_y,label='真实值')
# plt.scatter(mark,predict_y,label="预测值")
# plt.legend()
# plt.show()
#
#
...
# acc=np.mean(abs(predict_y-_y)<=1)
# print(acc)